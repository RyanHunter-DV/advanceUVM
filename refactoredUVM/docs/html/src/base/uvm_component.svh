...
// pragma protect begin
  extern virtual function void apply_named_config_settings (bit verbose = 0);
// pragma protect end
...
// pragma protect
// pragma protect begin
function void uvm_component::apply_named_config_settings (bit verbose=0);
  ...
  // recording_detail is 'magic' and not considered in the original field_array for apply_config_settings
  // but gets picked up if in the resources for that scope - add it here for explict search
  __m_uvm_status_container.field_array["recording_detail"] = 1;


  if(verbose)
    uvm_report_info("CFGAPL","applying configuration settings", UVM_NONE);

  foreach(__m_uvm_status_container.field_array[entry]) begin
    ...
    rq = rp.lookup_search_name(.scope(get_full_name()), .search_name(entry));
    ...
    if(verbose)
      uvm_report_info("CFGAPL",$sformatf("applying configuration to field %s from scope resource %s", name, r.get_scope()),UVM_NONE);

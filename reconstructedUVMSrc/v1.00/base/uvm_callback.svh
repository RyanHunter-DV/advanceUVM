...
class uvm_callbacks_base extends uvm_object; // {
  ...
  function bit check_registration(uvm_object obj, uvm_callback cb);
	...
	// @ryan, search through all existing uvm_callbacks, if is registered
	// in other callback pool
...
class uvm_typed_callbacks#(type T=uvm_object) extends uvm_callbacks_base; // {
  ...
  static this_type thisTypeInst;

	local function void _createThisTypeInst();
		thisTypeInst = new;
	endfunction
	local function void _createAnonymousCBQue();
		thisTypeInst.m_tw_cb_q = new("typewide_queue");
	endfunction
	local function void _initThisTypeInst; // {
		if(thisTypeInst == null) begin // {
			void'(super_type::initialize());
			_createThisTypeInst();
			_createAnonymousCBQue();
		end // }
	endfunction // }
  ...
  // @ryan, if this_type is parent of obj, then return 1
  // a.TCanbeCastedFrom(b)
	virtual function bit objSupports(uvm_object obj);
		T objType;
		if (obj == null) return 1;
		return($cast(objType,obj));
	endfunction
  ...
  virtual function uvm_queue#(uvm_callback) m_get_tw_cb_q(uvm_object obj);
	...
	// @ryan, find cb from q according to the name
...
class uvm_callbacks #(type T=uvm_object, type CB=uvm_callback)
  ...
  local static this_type thisInst;

  // typeinfo
  static uvm_typeid_base objectTypeid;
  static uvm_typeid_base callbackTypeid;

  static string objectTypeName;
  static string callbackType;
  ...
  // ---
	...
	static function this_type getStaticInst(); // {

		if (thisInst == null) begin // {
			uvm_typeid_base baseCallbackTypeid;
			// @ryan, directly call uvm_callbacks_base::initialize
			void'(super_type::initialize());

			baseCallbackTypeid = uvm_typeid#(uvm_callback)::get();
			callbackTypeid = uvm_typeid#(CB)::get();
			objectTypeid   = uvm_typeid#(T)::get();
			thisInst = new;

			if (baseCallbackTypeid == callbackTypeid) begin // {
				// @ryan, if input CB of this class is uvm_callback
				// @ryan, cast thisClassType ot baseClassType
				$cast(baseInst,thisInst);
				// The base inst in the super class gets set to this base inst
				typedInst = baseInst;
				uvm_typeid_base::callbackPools[objectTypeid] = thisInst;
				uvm_typeid_base::typeids[baseInst] = objectTypeid;
			// }
			end else begin // {
				// @ryan, if not, then set baseInst from getInst
				// @ryan, set baseInst as uvm_callbacks#(T,uvm_callback)
				baseInst = uvm_callbacks#(T,uvm_callback)::getStaticInst();
				// @ryan, push this inst into baseInst's queue, so in baseInst
				// there will be all static insts in baseInst
  ...
  // Register valid callback type
	...
	function void _setObjTypeName(string tname=""); // {
		objectTypeName = tname;
		super_type::objectTypeName = tname;
		objectTypeid.typename = tname;
	endfunction // }
	function void _setCBTypeName(string cbname); // {
		m_cb_typename = cbname;
		// @ryan, set typename of uvm_typeid#(CB)'s static object
		callbackTypeid.typename = cbname;
	endfunction // }
	// @ryan, set the static obj of this class's typename/cbname/m_registered
	// information
	static function bit registerObjAndCB(string tname="", cbname=""); // {
		// @ryan, get the static inst of this class type
		this_type inst = getStaticInst();

		// @ryan, set member typename, this class, uvm_typed_callbacks#(T),
		// and set the type name of uvm_typeid#(T)'s static object.
		_setObjTypeName(tname);
		_setCBTypeName(cbname);

		// @ryan, set registered flag.
		inst.m_registered = 1;

		return 1;
	endfunction

	virtual function bit isRegistered(uvm_object obj, uvm_callback cb);
		if(CBAvailable(cb) && objAvailable(obj)) begin
			return m_registered;
		end
		// return 0
	endfunction
	...
	// @ryan, if this_cb is same or parent of input cb, then return 1
	virtual function bit CBAvailable(uvm_callback cb);
		CB this_cb;return($cast(this_cb,cb));
	endfunction
  ...
  // @uvm-ieee 1800.2-2017 auto 10.7.2.3.1
	static function void add(T obj, uvm_callback cb, uvm_apprepend ordering=UVM_APPEND); // {
		...
		void'(get()); // incase never called the get()

		// @ryan, if cb is null, arrange some kind of names and report error
		if (cb==null) begin // {
			if (obj==null) nm = "(*)"; // @ryan, cb&obj all null
			else nm = obj.get_full_name(); // @ryan, use obj's full name
		...
		// @ryan, check registration in baseCBInst
		if (!m_base_inst.check_registration(obj,cb)) begin // {
			// @ryan, if not registrated

			// @ryan, get nm from obj, or use (*)
			if (obj==null) nm = "(*)";
			else nm = obj.get_full_name();

			// @ryan, get type name
			if (m_base_inst.m_typename!="") tnm = m_base_inst.m_typename;
			else if(obj != null) tnm = obj.get_type_name();
			else tnm = "uvm_object";
		...
		if(obj == null) begin // {
			// @ryan, add of unknown obj will be added to tw_cbs
			if (m_cb_find(m_t_inst.m_tw_cb_q,cb) != -1) begin // {
				// @ryan, already exists this cb
				if (m_base_inst.m_typename!="")tnm = m_base_inst.m_typename;
				else tnm = "uvm_object";
			...
			// }
				...
				// @ryan, report trace info if traceEnabled
		...
		// }
			...
			if(q.size() == 0) begin // {
				...
				if($cast(o,obj)) begin // {
					// @ryan, obj is derivative of report_object, then get all
					// typewide cbs from base report_object,callback class, and
					// push to q
  ...
  // @ryan, add cb to all T's derivatives components and matches the name
  ...
  // @ryan, append all matched cbs to the given ref queue
  static function void get_all ( ref CB all_callbacks[$], input T obj=null );
    uvm_queue#(uvm_callback) q;
    CB cb;
...
// @ryan, just kind of a wrapper for easily use the callback feature
...
class uvm_callback extends uvm_object; //{
  ...
  // @ryan, return the original m_enabled, and then flush the new value into
  // m_enabled

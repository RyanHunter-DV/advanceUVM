...
`endif //UVM_ENABLE_DEPRECATED_API
  ...
  function void set_scope (uvm_resource_base rsrc, string scope); 
    ...
    uvm_resource_types::rsrc_q_t rq; // @ryan, uvm_queue#(uvm_resource_base) rq
    string name;
    ...
    name = rsrc.get_name(); // @ryan, get the name from set_name or new
    if ((name != "") && rtab.exists(name)) begin // {
	// @ryan, if already called set_scope
      rq = rtab[name];
    ...
    if (rq == null) 
       rq = new(name); // @ryan, create a new rq

    // Insert the resource into the queue associated with its name.
    // Insert it with low priority (in the back of queue) .
    rq.push_back(rsrc); // @ryan, push_back into rq.queue

    rtab[name] = rq; // @ryan, then create a new array indexed by name of rtab
